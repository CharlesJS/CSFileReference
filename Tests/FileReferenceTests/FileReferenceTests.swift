...
// It seems FoundationEssentials doesn't include `Bundle`
import Foundation
...
@Suite(.serialized) struct FileReferenceTests {
    ...
    @Test(.serialized, arguments: fixtures) func testSlice(fixture: Fixture) async throws {
        ...
        let ref = try await fixture.constructor()
        try await unwrap(ref)
        _ = ref.self // keep ref from being deallocated before test is done
    }
